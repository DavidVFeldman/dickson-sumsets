...
The development is rebuilt on every revision by a public continuous
integration service, which compiles it against the pinned library
version and rejects the build should any result acquire a
\texttt{sorry} or an axiom beyond \texttt{propext},
\texttt{Classical.choice} and \texttt{Quot.sound}; the source, the
build logs and the axiom report are available in the archived
...
\emph{Dickson's conjecture and infinite multidimensional sum-sets
within the primes: paper and Lean~4 formalization},
Zenodo, \texttt{doi:10.5281/zenodo.21880269}.
